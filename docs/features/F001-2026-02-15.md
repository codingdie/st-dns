...
#### 6.2 规则匹配逻辑
- **匹配规则**:
  - `pattern` 支持精确匹配和 `*.` 前缀通配符
  - `pattern` 以 `re:` 开头时，后续内容按 ECMAScript 正则表达式完整匹配域名
  - `regex` 字段仍支持 ECMAScript 正则表达式，供已有配置继续使用
  - 每条规则只能配置 `pattern` 或 `regex` 其中之一，`re:` 与 `regex` 也不能同时配置
  - 非法正则表达式会被记录并跳过，不影响服务启动
  - 按配置顺序匹配
